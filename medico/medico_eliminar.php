...
try {
    ...
    $medicoInfo = getMedicoInfo($conn, $id);

    $sentencia = $conn->prepare("DELETE FROM medico WHERE id = ?;");
    ...
    if ($resultado === TRUE) {
        ...
        $event = "Médico eliminado: " . $medicoInfo['nombre'];
...
function getMedicoInfo($conn, $id)
{
    $sentencia = $conn->prepare("SELECT nombre FROM medico WHERE id = ?");
    $sentencia->bind_param("i", $id);
    $sentencia->execute();
