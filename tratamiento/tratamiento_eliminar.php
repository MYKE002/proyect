...
try {
    ...
    $tratamientoInfo = getTratamientoInfo($conn, $id);

    $sentencia = $conn->prepare("DELETE FROM tratamiento WHERE id = ?;");
    ...
    if ($resultado === TRUE) {
        ...
        $event = "Tratamiento  eliminado: " . $tratamientoInfo['nombre'];
...
function getTratamientoInfo($conn, $id)
{
    $sentencia = $conn->prepare("SELECT nombre FROM tratamiento WHERE id = ?");
    $sentencia->bind_param("i", $id);
    $sentencia->execute();
